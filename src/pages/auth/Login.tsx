...
const successPath: Record<PortalRole, string> = {
  admin:     '/admin',
  wholesale: '/wholesale',
  retail:    '/retail',
};
...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      const destination = successPath[user.role as PortalRole] ?? '/';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // No expectedRole — backend decides the role
      ...
      <div className="flex flex-col justify-center w-full md:w-1/2 px-6 py-12 sm:px-10 lg:px-14 bg-white order-2 md:order-1">
        <div className="w-full max-w-sm mx-auto">
          ...
          <div className="flex items-center gap-3 mb-10">
            <div className="login-panel--default w-11 h-11 rounded-xl flex items-center justify-center shadow-md flex-shrink-0 bg-orange-500">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 tracking-wide mt-0.5">Employee Portal</p>
            </div>
          </div>
          ...
          <p className="text-gray-400 text-sm mb-8">Sign in to access your portal</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl text-white font-semibold text-sm tracking-wide
                bg-orange-500 hover:bg-orange-600 active:scale-[0.98]
                shadow-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed
