...
const Login: React.FC = () => {
  ...
  // Navigate AFTER auth state is committed to React state.
  // Calling navigate() directly inside handleSubmit fires before
  // the setAuthState() in AuthContext re-renders — useEffect waits for it.
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success → useEffect above handles navigation once isAuthenticated flips
    } catch {
        ...
        <div className="bg-gray-800 rounded-2xl shadow-2xl overflow-hidden border border-gray-700">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-3 bg-gradient-to-r ${config.gradient} text-white font-semibold rounded-lg hover:opacity-90 transition-opacity shadow-lg disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2`}
            >
              {loading ? (
                <>
